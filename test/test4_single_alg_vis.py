...
def pso_opt(D, pop_size, max_iter, pop_max, pop_min, V_max, V_min, fitness_function, weight=None):
	...
	AlgorithmVisual(opt.recoder, evaluator, pop_min_, pop_max_)
...
def rqpso_opt(D, pop_size, max_iter, pop_max, pop_min, fitness_function, alpha=None, beta=None):
	...
	opt = Optimizer(max_iter, swarm, evaluator=fitness_function, rec_step=50, is_record=True)            # 优化算法初始化
	if alpha and beta:
		opt.fit(alpha, beta)
	else:
		opt.fit()           # 优化求解

	AlgorithmVisual(opt.recoder, evaluator, pop_min_, pop_max_, is_save=False)             # 结果可视化
